...
Given two hermitian operators $\hat{Q}$, $\hat{R}$ one has that 
\noindent\begin{align*}
...
\ptitle{Remark on Conjugation Rules}
...
\end{equation*}
i.e.\ one can ``separate'' complex conjugates of operators and functions without permutation which does not hold for operators and operators.
...
\noindent\begin{align*}
    \sigma^2     & = \langle \Psi|{(\hat{Q} - q)}^2 \Psi\rangle = 0 \\
    \hat{Q} \Psi & = q \Psi
\end{align*}
