...
\end{equation*}
of k-space for box volume $V$. This can be shown using the unit cuboid in k-space i.e.\ with $n_x=1,n_y=1,n_z =1$ and 
\begin{equation*}
    k_x k_y k_z = \frac{\pi^3}{l_x l_y l_z}
\end{equation*}

\newpar{}
\ptitle{Remarks}
\begin{itemize}
    \item One assumes noninteraction electrons and an empty box ($V=0$).
    \begin{itemize}
        \item Too simple to explain more advanced phenomena.
    \end{itemize}
    \item More precise model: Nuclei located on a periodic lattice and are modelled using the periodic potential model:~\ref{pot_models}.
\end{itemize}
...
\end{equation*}
and changes with the energy. It is denser for bottom states.From this, the spacing between energy levels results:
\noindent\begin{equation*}
    \Delta E = \frac{1}{D(E)}
\end{equation*}

\subsection{Potential Models}\label{pot_models}
...
With periodic boundary conditions (assuming wrap-around at boundaries)
\noindent\begin{equation*}
    \psi(x+Na) = \psi(x)
\end{equation*}
and Bloch's theorem
\noindent\begin{align*}
    ...
    1                    & =e^{iKNa}
\end{align*}
As $KNa\overset{!}{=} j\cdot 2 \pi,\;j\in \mathbb{Z}$ the quantization of $K$ can be found:
\noindent\begin{equation*}
    K=\frac{2\pi j}{Na}
\end{equation*}
...
\end{equation*}
which assumes that electrons reside near the nuclei. Hence, the latter are located at half-integer multiples of $a$.
Solving the 1D TISE (assuming lattice of ISW) for this potential results in
\noindent\begin{equation*}
...
\begin{itemize}
    \item Electrical current implies particle motion
    \begin{itemize}
        \item Increase of (kinetic) energy!
        \item Therefore, for conduction, free energy levels right above $E_f$ are needed.
    \end{itemize}
\end{itemize}